(function () {
  ...
  const API = {
    bootstrap: "/api/knowyourself/bootstrap",
    start: "/api/knowyourself/start",
    answer: "/api/knowyourself/answer",
    submit: "/api/knowyourself/submit",
  };
  ...
  const ui = {
    ...
    traitChips: document.getElementById("kyTraitChips"),
  ...
  function renderTraitChips(traits) {
    if (!ui.traitChips) return;
    ui.traitChips.innerHTML = "";
    const list = Array.isArray(traits) ? traits : [];

    list.forEach((trait, index) => {
      const chip = document.createElement("span");
      const group = trait.trait_group === "positive" ? "positive" : trait.trait_group === "negative" ? "negative" : "neutral";
      chip.className = `ky-trait-chip is-${group}`;
      chip.textContent = trait.trait;
      chip.style.animationDelay = `${Math.min(index * 45, 420)}ms`;
      ui.traitChips.appendChild(chip);
    });
  }
